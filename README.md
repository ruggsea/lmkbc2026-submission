...
| relation | elicitation | aggregation |
|---|---|---|
| countryLandBordersCountry | few-shot completion (3 hand-picked demonstrations), then collectivity injection | keep neighbour if it appears in ≥0.35 of draws |
| companyTradesAtStockExchange | many-shot listing prompt with explicit abstention, k=64 demonstrations drawn from official train | agreement ≥0.35 over scaffold-complete draws |
| hasArea | many-shot lead-sentence prompt, k=100 demonstrations log-stratified from a 331-entity bank, query removed | cluster in log10 space, w=0.05, median of largest cluster; a second geographic-entry pool overrides when the two medians differ by more than w |
| personHasCityOfDeath | prime completion ∪ CoT self-consistency, yes/no gate | modal city if vote share ≥ threshold |
| hasCapacity | REGION enumerate-then-pick | numeric cluster vote, r=0.025 |
| awardWonBy | list completion | keep name if it appears in ≥0.46 of draws |
